...
function GalleryCard({ v, index, onOpen }: { v: Video; index: number; onOpen: () => void }) {
  ...
  return (
    <Reveal delay={index * 80}>
      <TiltCard className="h-full" max={6}>
        <button
          onClick={onOpen}
          onMouseEnter={v.file ? enter : undefined}
          onMouseLeave={v.file ? leave : undefined}
          className={cn(
            ...
            <div className="relative aspect-video overflow-hidden">
              ...
              {v.file && (
                <video
                  ...
                  preload="none"
                  aria-hidden="true"
                  className="absolute inset-0 h-full w-full object-cover opacity-0 transition-opacity duration-500 group-hover:opacity-95"
                />
              )}
  ...
  return (
    <section id="video" className="relative z-10 mx-auto max-w-7xl scroll-mt-24 px-5 py-24 md:px-8">
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {videos.map((v, i) => (
          <GalleryCard key={v.id} v={v} index={i} onOpen={() => open(v)} />
        ))}
      </div>
