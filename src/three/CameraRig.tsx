...
export function CameraRig() {
  ...
  useFrame((state, delta) => {
    ...
    // punch toward the core on the drop
    const targetZ = 9 - progress * 3.4 - signal.beat * 0.22 - signal.drop * 0.5
    ...
    const k = signal.drop
    if (k > 0.001) {
      camera.position.x += Math.sin(t * 57.3) * 0.22 * k
      camera.position.y += Math.cos(t * 61.7) * 0.17 * k
    }
    camera.lookAt(0, 0.3 + progress * 0.6, 0)

    // cinematic roll + FOV breathing (applied after lookAt)
    camera.rotation.z = Math.sin(progress * Math.PI * 2) * 0.05 + pointer.x * 0.02 + Math.sin(t * 49.1) * 0.04 * k
